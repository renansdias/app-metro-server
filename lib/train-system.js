		...
		db.on('open', function() {
			...
			Train
				...
				.exec(function(err, docs) {
					 ...
					 * class.
					 */
					var trainDirection = this.stationNetwork.getDirectionForClosestTrain(origin, destination);

					/*
					 * Then, we need to determine which line the closest train should be
					 * on.
					 */
					var lineForClosestTrain = this.stationNetwork.getStartLineForPath(origin, destination);
					 ...
					 * 
					 * 1 - If the origin station is neither the first station
					 ...
					 *
					 * 2 - If the origin station is the first one, the closest
					 *     train will be the one that has the lowest index in the
					 *     trains array
					 *
					 * 3 - If the origin station is the last one, the closest
					 * 	   train will be the one that has the highest index in
					 *	   the trains array.
					 */
					var line = trains[0].train.line;

					if (locationIndexForStation === 0) {
						callback(trains[0]);

						return;
					} else if (locationIndexForStation === (self.locations[line].length - 1)) {
						callback(trains[trains.length - 1]);
