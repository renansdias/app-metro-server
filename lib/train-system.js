...
module.exports = function TrainSystem(stationNetwork) {
	...
	this.getNextLocationForTrain = function(train) {
		...
		this.locations[train.line].some(function(element, index, array) {
	...
	this.getClosestTrainTo = function(origin, destination, callback) {
		 ...
		 * origin is Sé and destination is Carrão, the train should
		 * be moving backwards.
		 * In order to know the direction, we'll use the Station Network
		 * class.
		 */
		var direction = this.stationNetwork.getDirectionForStations(origin, destination);
