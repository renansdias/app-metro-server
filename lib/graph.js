module.exports = function Graph() {
	 ...
	 * If weight is passed, we'll use it as the weight
	 * between v and w. Note that this "weight" could be
	 * anything: could be distance between nodes, transfer speed
	...
	this.addEdge = function(v, w, weight) {
		...
		if (this.adjacencyList[v.vertexId].map(function(e) {return e.vertex}).indexOf(w.vertexId) === -1) {
			this.adjacencyList[v.vertexId].push({vertex: w.vertexId, weight: weight || Number.POSITIVE_INFINITY});
			this.adjacencyList[w.vertexId].push({vertex: v.vertexId, weight: weight || Number.POSITIVE_INFINITY});
